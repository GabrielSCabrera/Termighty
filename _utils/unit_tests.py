from ..Termighty import Char, Charray, Terminal
from ..Charray import display_logo, big_letter, big_word, load_Charray, delete_Charray
...
def run_unit_tests():
    ...
    exceptions.check_type_function(int, 1)
    test("Multiple Type Success")
    exceptions.check_type_function((int,float), 1.0)
    test("Single Type Fail")
    try:
        exceptions.check_type_function(int, "a")
    except TypeError:
        pass
    test("Multiple Type Fail")
    try:
        exceptions.check_type_function((bool, slice), 2)
    except TypeError:
        pass
